...
export function GalleryCorridor({ onComplete }: { onComplete?: () => void }) {
  ...
  useEffect(() => {
    ...
    // Smooth, unhurried 8-second 3D Corridor walkthrough experience
    const duration = 8000;
    const animate = (time: number) => {
      ...
      if (currentProgress < 1) {
        ...
        // Elegant, smooth fade out at end of passage
        setTimeout(() => {
          handleSkip();
        }, 800);
      }
  ...
  const handleSkip = () => {
    ...
    setTimeout(() => {
      setIsVisible(false);
      if (onComplete) onComplete();
    }, 1000); // 1.0s smooth ease transition
  };
  ...
  // Camera translation along Z-axis: from Z = -2000px to Z = 200px
  const cameraZ = -2000 + progress * 2100;

  // Real artwork items from CSV Google Photos dataset
  const leftFrames = [
  ...
  return (
    ...
    >
      ...
      <div className="corridor-viewport w-full h-full relative flex items-center justify-center">
        {/* Subtle Vignette & Gallery Lighting overlay */}
        ...
        >
          {/* Left Wall Framed Artworks (Real CSV Photos) */}
          ...
          {/* Right Wall Framed Artworks (Real CSV Photos) */}
          ...
          {/* Vanishing Point Focal Title — Positioned cleanly so it's NEVER hidden */}
          <div
      ...
      {/* Persistent Bottom Bar & Enter Button */}
